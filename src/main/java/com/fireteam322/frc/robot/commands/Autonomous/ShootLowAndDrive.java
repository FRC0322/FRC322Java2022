...
public class ShootLowAndDrive extends CommandBase {
	 ...
	 */
	public ShootLowAndDrive(Chassis chassis, Intake intake, Feeder feeder, Shooter shooter) {
		m_chassis = chassis;
		m_intake = intake;
		m_feeder = feeder;
		m_shooter = shooter;
	...
	public void execute() {
		if (Timer.getFPGATimestamp() < (startTime + 1.0)) {
			m_shooter.run(Constants.LOW_SHOOTER_SPEED);

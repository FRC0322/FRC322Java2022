...
public class AddressableLEDs extends SubsystemBase {
  ...
  public void automaticLEDSetter() {
    if (DriverStation.isEStopped()) m_blinkRate = Constants.ESTOP_BLINK_RATE;
    else if (DriverStation.isDisabled()) m_blinkRate = Constants.DISABLED_BLINK_RATE;
    else if (DriverStation.isAutonomous()) m_blinkRate = Constants.AUTONOMOUS_BLINK_RATE;
    else if (DriverStation.isTeleop()) m_blinkRate = Constants.TELOP_BLINK_RATE;
    else m_blinkRate = 1.0;

...
public class Chassis extends SubsystemBase {
  /**
   * The Chassis subsystem incorporates the sensors and actuators attached to the robot's chassis.
   * It includes the four drive motors (two on each side), quadrature encoders connected directly to
   * each sides gearbox (read through the front TalonSRX for each side), and a navX-MXP IMU.
   */
  ...
  public boolean isHeadingReliable() {
    if (m_imu.isMagnetometerCalibrated() && !(m_imu.isMagneticDisturbance())) return true;
    else return false;
  }
  ...
  public float getCompassHeading() {
    if (isHeadingReliable()) return m_imu.getCompassHeading();
    else return 0.0f;
  }
  ...
  public float getHeading() {
    if (isHeadingReliable() && !isCalibrating()) return m_imu.getFusedHeading();
    else return 0.0f;
  }
